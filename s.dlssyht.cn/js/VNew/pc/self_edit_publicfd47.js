...
function show_botton(tag){
	...
	if(ev123_no_edit==1){
		...
		$.post('../self_define/ajax_set_info.html',{type:12,qu_name:tag,u:UTCTimeDemo()},
			function(data){
				var tmp_arr2=data.split('###');
				if(tmp_arr2[3]==20 || tmp_arr2[3]==21 || tmp_arr2[3]==22){
					//表单标签
					tmp_url='own_set_form.html';
				}else if(tmp_arr2[3]==12){
					//类别标签项
					tmp_url='own_set_index_tag.html';
				}else{
					if(tmp_arr2[0]==1){//是切换项下的
						if(tmp_arr2[1]==1){//表中已经存在数据
							tmp_url='own_qiehuan_list.html';
						}else{
							tmp_url='own_set_qiehuan.html';
						}
					}else{
						if(tmp_arr2[1]==1){//表中已经存在数据
							if(tmp_arr2[2]>0){
								tmp_url='own_index_edit.html';
								edit_id=tmp_arr2[2];
							}else{
								tmp_url='own_index_list.html';
							}
						}else{
							tmp_url='own_set_index.html';
						}
					}
				}
			}
...
function show_system(tag,id){
	...
	if(tag=='top'){
		tmp_url='own_set_top.html';
		title='头部信息';
	}
	if(tag=='foot_doc'){
		tmp_url='own_add_foot_doc.html';
		title='底部文章';
		tmp_height = 450;
	}
	if(tag=='linksite'){
		tmp_url='add_link_site.html';
		title='友情链接';
	}
	if(tag=='foot'){
		tmp_url='add_foot.html';
		title='底部信息';
	}
	if(tag=='banner'){
		tmp_url='own_add_banner.html';
		title='编辑模块';
		tmp_param+='&channel_id='+channel_id;
	}
	if(tag=='bbs'){
		tmp_url='add_bbs_class.html';
		title='编辑模块';
		tmp_param='is_frame=4&channel_id='+channel_id;
	}
	if(tag=='add_module'){
		tmp_url='own_add_module.html';
		title='编辑模块';
		tmp_param+='&channel_id='+channel_id+'&id='+id;
	}
	if(tag=='Container'){
		tmp_url='own_channel_style.html';
		title='设置版式';
		tmp_param+='&channel_id='+channel_id;
		tmp_width = 640;
		tmp_height = 320;
	...
	if(tag=='logo'){
		tmp_url='own_top_set.html';
		title='头部设置';
	}
	if(tag=='navi'){
		tmp_url='own_navi_set.html';
		title='栏目导航设置';
	}
	if(tag=='qu_banner'){
		tmp_url='own_banner_set.html';
		title='焦点图设置';
	}
...
function set_qu_edit(){
	...
	if(channel_id && channel_type!=7 && channel_type){
		if(channel_type==15 || channel_type==10 || channel_type==11 || channel_type==14){
			...
			var tmp_url="user_make_contentaff1.html?channel_id="+channel_id;
		}else{
			var tmp_str='';
			if(channel_type==19 || channel_type==18 || channel_type==20){
				var tmp_url="user_make_contentaff1.html?channel_id="+channel_id;
			}else if(channel_type==4){//博客
				var tmp_url="user_make_dom2092.html?dom_id=4";
			}else if(channel_type==31){//百科
				var tmp_url="user_make_dom6976.html?dom_id=32";
			}else if(channel_type==7){//论坛
				var tmp_url="user_make_dombddc.html?dom_id=14";
			}else if(channel_type==32){//知道
				var tmp_url="user_make_dom45c0.html?dom_id=31";
			}else if(channel_type==8){//相册
				var tmp_url="user_make_dom72cb.html?dom_id=21";
			}else if(channel_type==16){//下载
				var tmp_url="user_make_domc56d.html?dom_id=20";
			}else if(channel_type==30){//报价
				var tmp_url="user_make_dom3bb4.html?dom_id=1635";
			}else if(channel_type==34){//地图
				var tmp_url="user_make_dom2e3a.html?dom_id=19";
			}else if(channel_type==35){//免费建站
				var tmp_url="user_make_domd222.html?dom_id=1835";
			}else if(channel_type==36){//团购
				var tmp_url="user_make_dom7a75.html?dom_id=1845";
			}
		}
		if(tmp_url){
...
function delete_hand_module(id){
	...
	if(confirm("确定要删除吗？删除后将不可恢复！")){
		$.post('../self_define/ajax_set_info.html',{type:15,id:id,u:UTCTimeDemo()},
			function(data){
				data=parseInt(data);
				if(data==1){
					alert('操作成功！');
...
function hide_hide_qu(qu_id,is_cancel,tag){
	...
	$('#'+qu_id+'_hide img').attr('src','../../img.ev123.com/pic/editBtnAb308a4.jpg?ee');
	if(is_cancel!=1){
		$.post('../self_define/ajax_set_info.html',{type:13,qu_name:qu_id,show:0,u:UTCTimeDemo()},
			function(data){
				data=parseInt(data);
				if(data==0){
...
function show_hide_qu(qu_id,is_cancel,tag){
	...
	$('#'+qu_id+'_hide img').attr('src','../../img.ev123.com/pic/editBtnAb408a4.jpg?ee');
	if(is_cancel!=1){
		$.post('../self_define/ajax_set_info.html',{type:13,qu_name:qu_id,show:1,u:UTCTimeDemo()},
			function(data){
				data=parseInt(data);
				if(data==0){
					alert("设置失败，请检查是否已保存该模板！");
