...
function login_check(){
  ...
  if(!validatecode){
    ...
    var date = new Date();
    var url = "ajax_captcha442b.html?ajax=1&amp;captcha="+validatecode+"&t="+date.getTime();
...
function find_pwd_step_1(obj){
  ...
  var mobile_code  = $.trim($("#mobile_code").val());
  var url="ajax_pwd_action2716.html?username="+username_url;
  $.ajax({
      type: 'POST',
      url: url,
  ...
  $.ajax({
      ...
      success:function(data){
        if (data['res'] ==1){
          ...
          window.location.href="denglu2716.html?username="+username_url;
        }
      }
  })
}
  ...
  if(this_.attr("data-get")=="true"){
    ...
    if(type && mobile && user_id){
        ...
        $TN.onsuccess(function(){
            ...
            var url = "ajax_zhuce_code47d9.html?type="+type+"&userid="+user_id+"&mobile="+mobile+"&username="+user_name+"&tn_r="+tncode._mark_offset;
            $.post(url, function(data){
                if (data == 1) {
