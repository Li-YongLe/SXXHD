...
function submit_re_book(re_id){
	...
	if (parseInt(isTj) === 1) {
        ...
        var time=UTCTimeDemo();
        $.post("../ajax_set_info.html", {type:"9",des:sss,re_id:re_id,u:time,u_name:user_name},
               function(data){
                   if(data==0){
                   }else{
                       rm_book(data);
...
function get_re_book(re_id){
	re_id=parseInt(re_id);
	var url =  "../ajax_set_infoc32e.html?type=7&amp;u_name="+user_name+"&re_id="+re_id+"&u="+UTCTimeDemo();
	//alert(url);
